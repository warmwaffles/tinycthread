...
/// @brief TinyCThread version (major number).
#define TINYCTHREAD_VERSION_MAJOR 1
/// @brief TinyCThread version (minor number).
#define TINYCTHREAD_VERSION_MINOR 2
/// @brief TinyCThread version (full version).
#define TINYCTHREAD_VERSION 102
...
///       a compiler error), since compile-time TLS is not supported in the Mac
///       OS X executable format. Also, some older versions of MinGW (before GCC
///       4.x) do not support this directive, nor does the Tiny C Compiler.
///
/// @hideinitializer
...
/// @brief The results of a thread function
/// @ingroup tinycthread
typedef enum thrd_result {
    /// @brief The requested operation failed.
    thrd_error = 0,
    /// @brief The requested operation succeeded.
    thrd_success = 1,
    ...
    /// requested resource
    thrd_timedout = 2,
    /// @brief The requested operation is busy.
    ///
    /// The requested operation failed because a resource requested by a test
    /// and return function is already in use
    thrd_busy = 3,
    /// @brief The requested operation failed because it was unable to allocate
    ///        memory
    thrd_nomem = 4
} thrd_result_t;

typedef enum mtx_types {
    /// @brief A plain mutex
    mtx_plain = 0,
    /// @brief A timed mutex
    mtx_timed = 1,
...
typedef struct
{
    ...
    union
    {
        /// @brief Critical section handle (used for non-timed mutexes)
        CRITICAL_SECTION cs;
        /// @brief Mutex handle (used for timed mutex)
        HANDLE mut;
    } mHandle;
    /// @brief TRUE if the mutex is already locked
    int mAlreadyLocked;
    /// @brief TRUE if the mutex is recursive
    int mRecursive;
    /// @brief TRUE if the mutex is timed
    int mTimed;
} mtx_t;
#else
/// @brief Mutex
...
///
/// @return @ref thrd_success on success, or @ref thrd_error if the request
...
typedef struct
{
    /// @brief Signal and broadcast event HANDLEs.
    HANDLE mEvents[2];
    /// @brief Count of the number of waiters.
    unsigned int mWaitersCount;
    /// @brief Serialize access to mWaitersCount.
...
/// @brief Release any resources used by the given condition variable.
///
/// @param cond A condition variable object.
...
/// @brief Wait for a condition variable to become signaled.
///
...
/// @param cond  A condition variable object.
/// @param mtx   A mutex object.
/// @param ts    A point in time at which the request will time out (absolute
///              time).
///
/// @return  thrd_success upon success, or thrd_timeout if the time specified in
///          the call was reached without acquiring the requested resource, or
...
///
/// @return The thread return value, which can be obtained by another thread by
/// using the @ref thrd_join() function.
...
/// @brief Dispose of any resources allocated to the thread when that thread
///        exits.
///
...
/// @brief Destructor function for a thread-specific storage.
///
/// @param val The value of the destructed thread-specific storage.
typedef void (*tss_dtor_t)(void* val);

/// @brief Create a thread-specific storage.
///
...
///         could not be honored.
///
/// @note On Windows, the `dtor` will definitely be called when
...
/// @brief Delete a thread-specific storage.
///
...
/// @brief Get the value for a thread-specific storage.
///
...
/// @brief Set the value for a thread-specific storage.
///
/// @param key The thread-specific storage identifier.
...
/// @brief Invoke a callback exactly once
///
/// @param flag Flag used to ensure the callback is invoked exactly once.
